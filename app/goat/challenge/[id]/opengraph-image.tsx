...
export const alt = "Can You Become the GOAT? — beat this fighter build";
...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = ch ? `${ch.creatorWins}-${ch.creatorLosses}` : "13-0";
  const name = ch && ch.game === "goat" ? ch.creatorName : "A challenger";
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", fontSize: 40, color: "#d4d4d8", marginTop: 8 }}>
          {name} built a {record} fighter · GOAT {goat}
        </div>
        <div
          style={{
            display: "flex",
